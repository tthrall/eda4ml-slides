...
format:
  revealjs:
    ...
    slide-number: true
    preview-links: auto
    footer: "Chapter 5: Sampling and Study Design"
    incremental: false
    code-fold: false
    echo: true
---
...
::: {.callout-note}
## ML Connection
Training data is always a *sample* from the deployment distribution.
:::
...
2. **Experimental studies**: Learning from data you control  
...
# Simulated data matching the chapter's description
set.seed(42)
NB10_deficit <- rnorm(100, mean = 405, sd = 6.5)
```
...
# Add a couple of outliers to match chapter description
NB10_with_outliers <- c(NB10_deficit, 370, 440)
z_scores <- (NB10_with_outliers - mean(NB10_with_outliers)) / sd(NB10_with_outliers)
