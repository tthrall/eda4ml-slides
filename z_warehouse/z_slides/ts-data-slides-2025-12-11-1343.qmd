...
- The **time domain** asks: How does the past predict the future?
- The **frequency domain** asks: What structure generated this process?
...
#| fig-height: 4
acf1(sunspot.month, max.lag = 200, main = "")
```

- **Slow decay**: Strong persistence—the past predicts the future
- **Oscillation**: Peaks near lag 130 (~11 years) show cyclical structure
...
acf1(wn, main = "ACF")
mvspec(wn, log = "no", main = "Spectrum", col = 4)
```

- **Time series**: No visible pattern
- **ACF**: All lags near zero (within confidence bands)
- **Spectrum**: Roughly flat—no frequency dominates
...
2. **Check stationarity**: Does it wander? Transform if needed
