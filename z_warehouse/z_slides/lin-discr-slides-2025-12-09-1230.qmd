...
#| fig-cap: "When the direction of maximum variance is useless for classification"
#| fig-height: 4.5

set.seed(42)

# Create two classes: means differ along x, but variance is much larger along y
n <- 100
...
ggplot2::ggplot(pca_fail_data, aes(x = x, y = y, color = class)) +
...
geom_point(alpha = 0.6) +
  ...
  ggplot2::coord_fixed() +
  ggplot2::scale_color_manual(values = c("A" = "coral", "B" = "seagreen")) +
  ggplot2::labs(
...
Different means $\mu_k$, but same shape and orientation.
...
ggplot2::ggplot(ld1_scores, aes(x = LD1, fill = color)) +
  ...
  ggplot2::geom_vline(xintercept = 0, linetype = "dashed", color = "white") +
  ggplot2::labs(
...
wqual_lda <- MASS::lda(
  q_level ~ alcohol + volatile_acidity + sulphates,
  data = wqual_white
)
```
...
- [An Introduction to Statistical Learning (ISLR2)](https://www.statlearning.com/) — Chapter 4 covers LDA

- [The Elements of Statistical Learning](https://hastie.su.domains/ElemStatLearn/) — Chapter 4 for advanced treatment

- [MASS::lda() documentation](https://stat.ethz.ch/R-manual/R-devel/library/MASS/html/lda.html) — R's classic LDA implementation

- [discrim package](https://discrim.tidymodels.org/) — tidymodels interface to discriminant analysis
