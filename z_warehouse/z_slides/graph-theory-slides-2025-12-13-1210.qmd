...
|-------------|----------|
| **Prediction** | Who will connect next? What should we recommend? |
...
Two users are connected if they rated the same movie.

Edge weight = number of movies in common.
...
This projection enables standard graph algorithms on user similarity.
...
- **Nodes** have types (Person, Skill, Course, Role)
...
#| fig-cap: "Skill prerequisite structure (directed acyclic graph)"
#| fig-height: 5
...
**Question**: What's the shortest path from Programming to Statistical Learning?

. . .

```{r}
#| label: learning-path-demo

path <- shortest_paths(g_prereq, 
                       from = "Programming", 
                       to = "Statistical Learning",
                       output = "vpath")
cat(paste(names(path$vpath[[1]]), collapse = " → "))
```

. . .

This is **Dijkstra's algorithm** applied to skill acquisition.

. . .

The prerequisite DAG encodes *conceptual dependencies*—what you must understand before you can learn the next thing.

## Real-World Application: Workera

This isn't hypothetical. Companies like **Workera** (co-founded by Kian Katanforoosh, Andrew Ng as chairman) have built businesses on exactly this structure.
...
| **Spectral** | Eigenvectors of Laplacian | Principled, relates to graph cuts |
...
**Fiedler vector** (second eigenvector) minimizes the ratio cut—a principled way to split a graph.

# Graphs and Machine Learning

## Graphs as Feature Sources

Traditional ML on graphs: extract features, then apply standard methods.
...
For many problems, classical graph features remain competitive and interpretable.
...
3. **The same structure serves prediction and understanding**—link prediction exploits the patterns that community detection reveals.
...
- **ML connections**: Features from graphs, link prediction, GNNs
...
3. Using the LearningGraph, find the shortest learning path from Linear Algebra to Problem Formulation. What does this path suggest about skill development?
