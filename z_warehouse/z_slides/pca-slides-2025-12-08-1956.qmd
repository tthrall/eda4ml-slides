...
```{r}
#| label: wine-scores-mock
#| fig-cap: "Wine quality: PC1 vs PC2 scores colored by wine type"
#| fig-height: 4

# Simulated data to illustrate the point
set.seed(42)
n_red <- 1599
n_white <- 4898

wine_scores <- tibble(
  PC1 = c(rnorm(n_red, 2, 1), rnorm(n_white, -1, 1.2)),
  PC2 = c(rnorm(n_red, 0, 1), rnorm(n_white, 0, 1.2)),
  color = c(rep("red", n_red), rep("white", n_white))
)

ggplot(wine_scores, aes(x = PC1, y = PC2, color = color)) +
  geom_point(alpha = 0.3) +
  scale_color_manual(values = c("red" = "darkred", "white" = "gold3")) +
  labs(
