...
# Project onto users
user_proj <- igraph::bipartite_projection(g, which = "false")  # "false" = first type (users)
igraph::E(user_proj)$width <- igraph::E(user_proj)$weight * 2

igraph::V(user_proj)$color <- "#9B59B6"
igraph::V(user_proj)$size <- 25
...
The prerequisite DAG is fixed; your position on it varies. The "path to Statistical Learning" doesn't exist—only Alice's path, Bob's path, Carol's path.
