...
plot(spec_wn$freq, spec_wn$spec, type = "l", col = 4,
     xlab = "Frequency", ylab = "Periodogram",
     main = "White Noise: True Spectrum is Flat")
abline(h = 1, col = 2, lty = 2, lwd = 2)
legend("topright", legend = c("Periodogram", "True spectrum"), 
       col = c(4, 2), lty = c(1, 2), bw = "n")
```
...
spec.pgram(x, spans = c(5, 5), taper = 0.1)
```
...
plot(spec_raw$freq, spec_raw$spec, type = "l", col = gray(0.7),
     xlab = "Frequency", ylab = "Spectrum estimate",
     main = "Effect of Smoothing")
lines(spec_smooth$freq, spec_smooth$spec, col = 4, lwd = 2)
abline(h = 1, col = 2, lty = 2, lwd = 2)
legend("topright", legend = c("Raw periodogram", "Smoothed", "True"), 
       col = c(gray(0.7), 4, 2), lty = c(1, 1, 2), lwd = c(1, 2, 2), bw = "n")
```
...
R's `spec.pgram()` uses log scale by default.
...
astsa::arma.spec(ar = c(1.3, -0.7), main = "AR(2): φ₁ = 1.3, φ₂ = -0.7", col = 4)
```
...
abline(v = 1/132, col = 2, lty = 2)
text(1/132 + 0.01, max(log10(sunspots_spec$spec)) - 1, "~11 years", col = 2, adj = 0)
```
...
# Daily temperatures for Honolulu
# Using a subset of data for cleaner presentation
data(cardox)  # placeholder - would use actual HNL data
```
...
#| fig-cap: "Simulated daily temperature with annual cycle (placeholder for actual HNL data)"
#| fig-height: 4

# Simulate temperature data with annual cycle for demonstration
set.seed(123)
n_days <- 365 * 10  # 10 years
t_days <- 1:n_days
annual_cycle <- 5 * cos(2 * pi * t_days / 365)  # 5°F amplitude
noise <- arima.sim(list(ar = 0.7), n = n_days, sd = 2)
temp_sim <- ts(75 + annual_cycle + noise, frequency = 365)

astsa::tsplot(temp_sim, col = 4, ylab = "Temperature (°F)", main = "")
```

The annual cycle is visible but embedded in day-to-day variation.

## Temperature Spectrum

```{r}
#| label: fig-hnl-temp-spectrum
#| fig-cap: "Temperature spectrum: sharp peak at annual frequency"
#| fig-height: 4

temp_spec <- stats::spec.pgram(temp_sim, spans = c(7, 7), taper = 0.1, 
                                log = "yes", col = 4, main = "")
# Mark annual frequency
abline(v = 1/365, col = 2, lty = 2)
text(1/365 + 0.002, max(log10(temp_spec$spec)) - 1, "1 year", col = 2, adj = 0)
```
...
**Time domain** excels when:

- Goal is forecasting
- Interest is in short-term dependence
- Model building for prediction

. . .

**Frequency domain** excels when:

- Goal is identifying periodicities
- Interest is in cyclic structure
