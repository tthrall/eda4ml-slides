...
source(here::here("code", "wine_quality_uci.R"))
```

```{r}
#| label: wine-data

wq_data <- get_wine_quality_uci()
...
- Text: hundreds of thousands of words
...
| Chapter | Subspace | Determined by |
|---------|----------|---------------|
...
All three use **orthogonal projection**—they differ in how the target subspace is chosen.
...
From @McNeil_1977: violent crime rates per 100,000 population (1973)
...
`r format(wq_n_obs, big.mark = ",")` wines with 11 chemical properties:
...
ggplot2::ggplot(galton_ctr, ggplot2::aes(x = father, y = son)) +
  ggplot2::geom_point(alpha = 0.5, color = "steelblue") +
  ggplot2::geom_abline(intercept = 0, slope = pc1_slope, 
                       color = "coral", linewidth = 1.2) +
  ggplot2::geom_abline(intercept = 0, slope = pc2_slope, 
                       color = "gold", linewidth = 1.2) +
  ggplot2::coord_fixed() +
  ggplot2::annotate("text", x = 3, y = 3.5, label = "PC1", 
                    color = "coral", size = 5) +
  ggplot2::annotate("text", x = -3, y = 2, label = "PC2", 
                    color = "gold", size = 5) +
  ggplot2::labs(
...
pca_arrests <- arrests |>
  dplyr::select(Murder, Assault, Rape) |>
  scale() |>
  stats::prcomp(center = FALSE)

# Loadings
loadings_df <- pca_arrests$rotation |> 
  tibble::as_tibble(rownames = "crime") |>
  tidyr::pivot_longer(-crime, names_to = "PC", values_to = "loading")
```
...
ggplot2::ggplot(loadings_df, ggplot2::aes(x = crime, y = loading, fill = PC)) +
...
#| fig-cap: "Wine Quality: scree plot"
#| fig-height: 3.5
...
tibble::tibble(
  PC = seq_along(wq_pca$sdev),
  Variance = wq_pca$sdev^2
) |>
  ggplot2::ggplot(ggplot2::aes(x = PC, y = Variance)) +
  ggplot2::geom_line() +
  ggplot2::geom_point(size = 2) +
  ggplot2::scale_x_continuous(breaks = seq(2, 14, by = 2)) +
  ggplot2::labs(title = "Wine Quality: Variance by Principal Component") +
  ggplot2::theme_minimal()
```
...
loadings_df |>
  ...
  ggplot2::ggplot(ggplot2::aes(x = crime, y = loading, fill = crime)) +
...
```{r}
#| label: wine-scores
#| fig-cap: "Wine quality: PC1 vs PC2 scores colored by wine type"
#| fig-height: 4

wq_scores <- wq_pca$x |> 

  tibble::as_tibble() |> 
  dplyr::mutate(color = wq_data$color)

ggplot2::ggplot(wq_scores, ggplot2::aes(x = PC1, y = PC2, color = color)) +
  ggplot2::geom_point(alpha = 0.3) +
  ggplot2::scale_color_manual(values = c("red" = "darkred", "white" = "gold3")) +
  ggplot2::labs(
    title = "Wine Quality: PC1 Nearly Perfectly Separates Red and White",
    x = "PC1 score",
    y = "PC2 score"
  ) +
  ggplot2::theme_minimal()
...
- Two clusters along PC1
- Near-perfect correspondence with red/white labels
...
- Red and white wines differ chemically
- These differences create variance
- PC1 captures that variance
...
3. **Dimension reduction** — project onto first $r$ PCs to reduce noise and enable visualization
...
1. What determines the subspace in each method?
