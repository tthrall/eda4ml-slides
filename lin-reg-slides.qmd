...
ggplot() +
  # Feature space line
  geom_segment(aes(
    x = 0, y = 0, xend = 4, yend = 2), 
    color = "steelblue", linewidth = 1.5, 
    arrow = arrow(length = unit(0.3, "cm")
  )) +
  # y vector
  geom_segment(aes(
    x = 0, y = 0, xend = 2, yend = 3), 
    color = "coral", linewidth = 1.5, 
    arrow = arrow(length = unit(0.3, "cm")
  )) +
  # y-hat (projection)
  geom_segment(aes(
    x = 0, y = 0, xend = 2.4, yend = 1.2), 
    color = "gold", linewidth = 1.5, 
    arrow = arrow(length = unit(0.3, "cm")
  )) +
  # residual (perpendicular)
  geom_segment(aes(
    x = 2.4, y = 1.2, xend = 2, yend = 3 ), 
    color = "gray50", linewidth = 1, linetype = "dashed") +
  # right angle marker
  geom_segment(aes(
    x = 2.4, y = 1.2, xend = 2.28, yend = 1.44 ), 
    color = "gray50") +
  geom_segment(aes(
    x = 2.28, y = 1.44, xend = 2.16, yend = 1.32 ), 
    color = "gray50") +
  # Labels - use parse = TRUE instead of expression()
  annotate(
    "text", x = 4.2, y = 2, label = "col(X)", 
    size = 5, color = "steelblue" ) +
  annotate(
    "text", x = 1.7, y = 3.2, label = "y", 
    size = 6, color = "coral" ) +
  annotate(
    "text", x = 2.7, y = 0.9, label = "hat(y)", 
    parse = TRUE, size = 6, color = "gold" ) +
  annotate(
    "text", x = 2.6, y = 2.3, label = "epsilon", parse = TRUE, 
    size = 5, color = "gray50" ) +
  coord_fixed() +
  theme_void() +
  theme(
